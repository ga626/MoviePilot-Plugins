...
- 当前唯一正式产品是 `MediaGovernor`，目录 `plugins.v3/mediagovernor/`。它的目标是通用媒体治理：识别置信度、命名冲突、文件集完整性、硬链接条件和可诊断异常队列；禁止为单一影视作品写硬编码特例。`plugins.v3/mediagovernorvalidator/` 是正式重构前的一次性只读验证台，只能验证 V0-V8、导出脱敏回执并在合同移植后退役，不得获得媒体写入或修复能力。
