...
这是符合 MoviePilot V3 插件目录与索引规范的 MediaGovernor S3，当前版本为 `v0.3.4`，插件 ID 为 `MediaGovernor`，源码目录为 `plugins.v3/mediagovernor/`。

## 当前能做与不能做

- 已完成的 S1：`v0.1.0` 已在真实 MoviePilot V3 宿主验证发现、市场安装、配置页、详情页，以及“启用后运行 / 停用后恢复禁用”的生命周期；该稳定版本仍保持在 NAS 上禁用。
- S3 能做：默认关闭时不处理任何事件；启用后，新发生的 `TransferComplete` / `TransferFailed` 会进入队列，所有记录按规范媒体身份聚合为脱敏作品包。宿主提供历史分页接口时，会分批只读回溯已有失败整理历史，再每 15 分钟续扫；较早稳定宿主不提供该接口时，插件仍可正常加载并明确显示“历史回溯降级”，不会把缺失接口误变成安装失败。
