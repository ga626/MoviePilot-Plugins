...
- S3 候选能做：默认关闭时不处理任何事件；启用后只观察 `TransferComplete` / `TransferFailed`，以公开的只读历史接口补全元数据，按规范媒体身份聚合为脱敏作品包，并通过只读 API 显示队列。
...
- `v0.2.0` 的 `release` 为 `false`：它是本地候选，不会覆盖已验证并安装的 `v0.1.0`，也不会进入市场。
