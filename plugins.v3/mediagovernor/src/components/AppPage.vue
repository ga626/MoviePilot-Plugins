...
<template>
  <main class="governor-page">
    <header class="page-header">
      <div><h1>整理质量检查</h1><p>按作品汇总核对 MoviePilot 的整理历史。每次检查都会从当前历史记录重新开始；检查本身不会移动、删除、改名或创建文件。</p></div>
    ...
    <div v-if="selected" class="modal-backdrop" @click.self="closeIssue"><section class="modal" role="dialog" aria-modal="true" aria-label="整理检查结论"><header class="modal-header"><div><span class="modal-label">检查结论</span><h2>{{ titleFor(selected) }}</h2></div><button class="icon-button" aria-label="关闭" @click="closeIssue">×</button></header><section class="modal-section"><h3>这部作品的 {{ selected.record_count }} 条整理记录</h3><p>这里按影片合并展示。下面每项都是检查到的具体事实，不把历史字段当成实际文件状态。</p></section><section v-for="finding in selected.findings" :key="`${finding.status}-${finding.transfer_mode || 'none'}`" class="modal-section"><h3>{{ finding.title }}（{{ finding.count }} 条）</h3><p>{{ finding.detail }}</p></section><section v-if="selected.repairable_count" class="modal-section"><h3>可以自动处理什么？</h3><p>其中 {{ selected.repairable_count }} 条是原整理失败、且身份已确认的记录。整理方式待复核的成功记录不会被错误地拿去自动重建。</p><button v-if="!readyPlansFor(selected).length" class="primary wide" :disabled="loading" @click="prepareGroupPlans(selected)">{{ loading ? '正在生成方案…' : '一键生成修复方案（不改文件）' }}</button><button v-else class="primary wide" :disabled="loading" @click="pendingGroupRepair = selected">确认修复 {{ readyPlansFor(selected).length }} 条记录</button></section><details class="manual-guide"><summary>为什么这里不能直接一键修复所有项目？</summary><ol><li>“历史记录显示为复制/移动”只说明当时的记录方式，不等于实际文件现在不存在或损坏。</li><li>创建硬链接会写入媒体库；只有原整理明确失败且补建前检查通过的记录，才适合进入确认后的修复流程。</li><li>目录放错、缺集等问题需要宿主提供真实目录与剧集完整性证据；没有这些证据，工具不会猜测或误修。</li></ol></details></section></div>
