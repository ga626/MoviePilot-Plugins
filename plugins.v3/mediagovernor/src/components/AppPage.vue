...
<template>
  <main class="governor-page">
    <header class="page-header">
      <div><h1>整理质量检查</h1><p>这是整理流程的最后一道检查：逐条核对历史异常，找出需要处理的项目。检查不会移动、删除、改名或创建文件。</p></div>
      <button class="secondary" :disabled="loading" @click="refresh">{{ loading ? '正在读取…' : '更新进度' }}</button>
    </header>
    <p v-if="notice" class="notice" role="status">{{ notice }}</p>
    <section v-if="!hasRun" class="start-panel" aria-label="开始整理检查">
    ...
    <section v-else class="result-panel" aria-label="检查结果">
      <div class="result-heading"><div><h2>{{ completed ? '检查完成' : (running ? '正在检查' : '检查已暂停') }}</h2><p>已检查 {{ summary.checked || 0 }} / {{ summary.total || 0 }} 条历史记录。检查会逐条保存结果，刷新页面也不会丢失进度。</p></div><div class="header-actions"><button v-if="running" class="secondary" :disabled="loading" @click="pauseAudit">暂停检查</button><button v-if="!completed && !running" class="primary" :disabled="loading" @click="auditAll">继续检查</button></div></div>
      <div class="progress-wrap" aria-label="检查进度"><div class="progress-copy"><strong>{{ progress }}%</strong><span v-if="running">正在核对第 {{ (summary.checked || 0) + 1 }} 条</span><span v-else>本轮进度已保存</span></div><div class="progress-track"><div class="progress-bar" :style="{ width: `${progress}%` }"></div></div></div>
      <div class="overview" aria-label="检查结果概览"><article><strong>{{ summary.actionable || 0 }}</strong><span>可生成方案</span></article><article><strong>{{ summary.ready_for_preview || 0 }}</strong><span>等待核对</span></article><article><strong>{{ summary.blocked || 0 }}</strong><span>暂不能处理</span></article><article><strong>{{ summary.unresolved || 0 }}</strong><span>信息不足</span></article></div>
      ...
      <div v-else class="issues"><article v-for="card in cards" :key="card.history_id" class="issue-card"><div class="issue-copy"><span class="issue-type">{{ guideFor(card).label }}</span><h3>{{ titleFor(card) }}</h3><p>{{ guideFor(card).detail }}</p></div><button class="primary" @click="openIssue(card)">查看问题和方案</button></article></div>
    </section>
