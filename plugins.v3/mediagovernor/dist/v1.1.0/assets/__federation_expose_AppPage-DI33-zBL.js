...
const _hoisted_14 = { class: "chip" };
const _hoisted_15 = {
  key: 0,
  class: "empty"
};
const _hoisted_16 = { class: "state" };
...
const _hoisted_33 = { class: "audit-grid" };
const _hoisted_34 = {
  key: 0,
  class: "reason"
};
...
const _hoisted_38 = { class: "modal confirm" };
  ...
  return (_openBlock(), _createElementBlock("main", _hoisted_1, [
    ...
    _createElementVNode("section", _hoisted_5, [
      _createElementVNode("div", _hoisted_6, [
        ...
        _createElementVNode("span", null, _toDisplayString(run.value.completed) + "/" + _toDisplayString(run.value.total) + " · " + _toDisplayString(elapsed.value), 1)
      ]),
      ...
      (run.value.current)
        ? (_openBlock(), _createElementBlock("p", _hoisted_8, _toDisplayString(run.value.current) + "。结果会逐条出现，不必等待全部结束。", 1))
        : _createCommentVNode("", true),
    ...
    (notice.value)
      ? (_openBlock(), _createElementBlock("p", _hoisted_10, _toDisplayString(notice.value), 1))
      : _createCommentVNode("", true),
    ...
    (tab.value === 'identity')
      ? (_openBlock(), _createElementBlock("section", _hoisted_12, [
          _createElementVNode("div", _hoisted_13, [
            _cache[11] || (_cache[11] = _createElementVNode("div", null, [
              ...
              _createElementVNode("p", null, "从整包名称、季集和官方候选交叉判断。任何冲突都不会自动进入整理。")
            ], -1)),
            _createElementVNode("span", _hoisted_14, "已发现 " + _toDisplayString(sources.value.length) + " 个数据源", 1)
          ]),
          (!cards.value.length)
            ? (_openBlock(), _createElementBlock("p", _hoisted_15, "点击“开始检查”后，进度和每个来源包的结论会立即显示在这里。"))
            : _createCommentVNode("", true),
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(cards.value, (card) => {
            return (_openBlock(), _createElementBlock("article", {
              ...
              _createElementVNode("div", null, [
                _createElementVNode("span", _hoisted_16, _toDisplayString(card.state === 'confirmed' ? '已确认' : card.state === 'needs_selection' ? '待你选择' : card.state === 'insufficient' ? '资料不足' : '来源不可用'), 1),
                ...
                (card.evidence)
                  ? (_openBlock(), _createElementBlock("p", _hoisted_17, _toDisplayString(evidenceText(card.evidence)) + "。" + _toDisplayString(card.evidence.truncated ? '目录较深，已按安全上限抽样。' : '已完成包级抽样。'), 1))
                  : (_openBlock(), _createElementBlock("p", _hoisted_18, _toDisplayString(card.reason), 1)),
                (card.reason)
              ...
              }, _toDisplayString(card.state === 'confirmed' ? '查看判断' : '处理候选'), 9, _hoisted_20)
            ], 2))
          }), 128))
        ]))
      : (_openBlock(), _createElementBlock("section", _hoisted_21, [
          ...
          (!confirmed.value.length)
            ? (_openBlock(), _createElementBlock("p", _hoisted_22, "先在“找对作品”里得到已确认身份。冲突候选和资料不足的包不会出现执行入口。"))
            : _createCommentVNode("", true),
            ...
            (tab.value === 'identity')
              ? (_openBlock(), _createElementBlock(_Fragment, { key: 0 }, [
                  ...
                  (selected.value.evidence)
                    ? (_openBlock(), _createElementBlock("p", _hoisted_25, "包级摘要：" + _toDisplayString(evidenceText(selected.value.evidence)) + "。真实路径和完整文件名不会在这里展示或保存。", 1))
                    : _createCommentVNode("", true),
                  ...
                  (selected.value.candidates?.length)
                        ...
                        (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(selected.value.candidates, (candidate) => {
                          return (_openBlock(), _createElementBlock("article", {
                            ...
                            _createElementVNode("div", null, [
                              ...
                              _createElementVNode("p", null, "支持：" + _toDisplayString(candidate.reasons?.join('；') || '仅搜索命中'), 1),
                              (candidate.conflicts?.length)
                                ? (_openBlock(), _createElementBlock("p", _hoisted_27, "冲突：" + _toDisplayString(candidate.conflicts.join('；')), 1))
                                : _createCommentVNode("", true)
                            ]),
                            _createElementVNode("button", {
                              class: "secondary",
                    ...
                    : (_openBlock(), _createElementBlock("p", _hoisted_29, "没有足够的官方候选。这不代表文件为空，只表示当前公开数据源还不能确认。")),
                  (selected.value.state === 'confirmed')
                    ? (_openBlock(), _createElementBlock("p", _hoisted_30, "身份已确认。下一步可进入“整理正确”，先做官方零写入审计。"))
                    : _createCommentVNode("", true)
                ], 64))
              : (_openBlock(), _createElementBlock(_Fragment, { key: 1 }, [
                  ...
                  }, _toDisplayString(loading.value ? '官方审计中…' : '重新生成官方预览'), 9, _hoisted_31),
                  (audit.value)
                    ? (_openBlock(), _createElementBlock("section", _hoisted_32, [
                        ...
                        _createElementVNode("p", null, _toDisplayString(audit.value.target?.target_path ? '当前目录规则给出了唯一目标。' : '当前目录规则没有给出唯一目标。'), 1),
                        (audit.value.issues.length)
                          ? (_openBlock(), _createElementBlock("p", _hoisted_34, "已锁定执行：" + _toDisplayString(audit.value.issues.join('；')), 1))
                          : (_openBlock(), _createElementBlock("p", _hoisted_35, "预览完整：MoviePilot 负责清理命中的旧硬链接并创建新硬链接；来源文件不会被插件直接改动。")),
                        _createElementVNode("button", {
                          class: "primary",
                          disabled: loading.value || !audit.value.eligible,
                          onClick: _cache[3] || (_cache[3] = $event => (confirmRepair.value = true))
                        }, "确认重整此包", 8, _hoisted_36)
                      ]))
                    : _createCommentVNode("", true)
                ], 64))
          ])
...
const AppPage = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-61d0d754"]]);
