...
async function diagnoseBundles(items) { const result = new Map(), batchSize = 12; for (let offset = 0; offset < items.length; offset += batchSize) { const batch = items.slice(offset, offset + batchSize).map((item, index) => ({ id: `bundle-${offset + index + 1}`, evidence: bundlePayload(item.group, item.evidence).evidence, key: item.group.key })); try { const raw = dataOf(await props.api.post('plugin/MediaGovernor/bundle_analyze_batch', { items: batch.map(({ id, evidence }) => ({ id, evidence })) }, { feedback: 'silent' })); if (raw?.success === false) throw new Error(raw.message || '模型没有完成整包分析。'); const diagnoses = raw?.data?.diagnoses || raw?.diagnoses || {}; batch.forEach(item => result.set(item.key, diagnoses[item.id] || { error: '模型没有返回这个包的结构化判断。' })); } catch (error) { batch.forEach(item => result.set(item.key, { error: error?.message || '无法调用当前 MoviePilot 模型；仍会继续原生候选核验。' })); } } return result }
                  ...
                  (preview.value)
                    ? (_openBlock(), _createElementBlock("section", _hoisted_50, [
                        ...
                        (preview.value.issues.length)
                          ...
                          : (_openBlock(), _createElementBlock("p", _hoisted_53, "新硬链接预览完整。确认后只调用 MoviePilot 官方整理：它只会清理这条成功历史对应的旧媒体库硬链接，再从原始下载文件重建；不会删除下载原文件。")),
                        (!preview.value.receipt)
                          ? (_openBlock(), _createElementBlock("button", {
                            ...
                            }, "确认按此预览重建", 8, _hoisted_54))
                          : (_openBlock(), _createElementBlock("p", _hoisted_55, "本次已交给 MoviePilot 执行；请重新检查确认结果。")),
...
const AppPage = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-5af430d0"]]);
