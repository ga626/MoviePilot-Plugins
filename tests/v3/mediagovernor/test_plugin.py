...
def test_manifest_plugin_and_frontend_versions_are_synced() -> None:
    ...
    assert list(manifest["history"])[0] == "v1.1.0"
    assert 'plugin_version = "1.1.0"' in source
    assert 'return "vue", "dist/v1.1.0/assets"' in source
    assert "assetsDir: 'v1.1.0/assets'" in (ROOT / "plugins.v3/mediagovernor/vite.config.js").read_text(encoding="utf-8")
...
def test_plugin_is_a_thin_host_contract_without_private_backend_dependencies() -> None:
    ...
    assert plugin_class.get_render_mode() == ("vue", "dist/v1.1.0/assets")
...
def test_frontend_uses_injected_authenticated_api_and_all_recovery_endpoints() -> None:
    ...
    assert "maxDirs" in page and "maxDepth" in page
...
def test_repair_payload_uses_official_preview_then_confirmed_reorganize() -> None:
    ...
    assert "payload(selected.value, true, true)" in page
    assert "payload(selected.value, false, true)" in page
...
def test_release_sources_do_not_ship_legacy_private_governor() -> None:
    ...
    assert (ROOT / "plugins.v3/mediagovernor/dist/v1.1.0/assets/remoteEntry.js").is_file()
